...
const Card = ({ job }) => {
  ...
  const handleDelete = () => {
    if (confirm("Silmek istediğinizden Emin misiniz?")) {
      //api isteiği atmalıyız
      axios
        .delete(`http://localhost:3001/jobs/${job.id}`)
        //istek başarılı olrusa store den kaldır
        .then(() => {
          dispatch(deleteJob(job.id));
          toast.success("İşlem gerçekleşti");
        })
        //istek başarılı olmaz ise uyarı ver
        .catch((err) => toast.error("İşlem gerçekleşitken bir hata oluştu"));
    }
  };
  return (
    <div className="card">
